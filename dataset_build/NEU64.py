...
if config["few_shot"] == False:
    _, train_dataloader,_,val_dataloader = get_loader_NEU64(config, True)
...
def get_loader_NEU64(config, shuffle):
    image_path = config["image_path"]
    dataset = CustomDataset_NEU64(image_path,transform=transforms.ToTensor())
    train_dataset, validate_dataset=train_test_split_NEU64(dataset, config["validate_ratio"])
...
class CustomDataset_NEU64(Dataset):
    def __init__(self, image_folder, transform=None):
        ...
        for label, images in self.class_to_images.items():
            sampled = random.sample(images, min(200, len(images)))
...
def few_shot_data_loader_NEU64_transfer(config, shuffle=True):
    ...
    base_dataset = CustomDataset_NEU64(image_folder=image_folder, transform=transform)
...
def train_test_split_NEU64(base_dataset, test_size=0.3):
...
def few_shot_data_loader_NEU64(train_dataset, test_dataset, num_classes=5, num_support=5, num_query=15, batch_size=6, num_workers=1, pin_memory=False, shuffle=True):
...
def few_shot_data_loader_NEU64_whole(config,shuffle):
